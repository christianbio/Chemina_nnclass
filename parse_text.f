      ...
      subroutine split_text_to_list2(buffer,charsplit,textlist,n)
            character(len=2048) buffer,text1,text2,string
            character(len = 2048), dimension(64) :: textlist
            character(len=1) :: charsplit
            integer i,n

            string = justifyl(buffer)
            i = 0 
            do while(.true.)
               call split_text2(string,text1,text2,charsplit)
               i = i + 1
               textlist(i) = text1
               if(text2.eq.'  ') exit
               string = text2
...
! Find the first instance of whitespace.  Split label and data.
      ...
      subroutine split_text2(buffer,text1,text2,charsplit)
      implicit none
      character(len=2048) :: buffer,text2,text1
      character(len=1) :: charsplit
      integer pos,first_space
...
! Find the first instance of whitespace.  Split label and data.
      ...
      first_space = scan(buffer,charsplit)
      if(first_space.eq.0) first_space = 512

      pos = first_space
      text1 = buffer(1:pos-1)
      text2 = buffer(pos+1:)

      text1 = justifyl(text1)
      text2 = justifyl(text2)

      end subroutine split_text2
